...
Generic in-process cell runner. Owned by ``ml-eda`` and shared by
``evaluate-ml-pipeline`` (and any future skill that executes a jupytext
percent-format ``.py`` file cell by cell). Uses
...
cells contain (see `evaluate-ml-pipeline` § Audit and `ml-eda` § EDA
file contract).
