# Recursive Ruff Clean-Up

Iterative Ruff linting: autofix safe, then unsafe, review diffs, suppress only
when justified, repeat until clean.
...
- `ruff_runner` (optional): explicit Ruff command prefix (for example `uv run`,
  `ruff`, `python -m ruff`, `pipx run ruff`).
...
2. Prefer the project's environment manager (`pixi run ruff`, `uv run ruff`,
   `poetry run ruff`, etc.) per `python-env-manager`.
3. Else if `ruff` is available on `PATH`, use `ruff`.
4. Else if Python is available and Ruff is installed in that environment, use
   `python -m ruff`.
5. Else use any project-specific equivalent that invokes installed Ruff, or stop
   and ask the user.

Use the same resolved `ruff_cmd` for all `check` and `format` commands in the
workflow.

```bash
# Full project with defaults
ruff check

# One folder
python -m ruff check src/models

# Override to skip docs and TODO-like rules
...
If there are multiple valid solutions at any step, always ask the user before
proceeding. Do not choose silently between equivalent options.

### 6. Suppression Decision (`# noqa`)

Use suppression only when all conditions are true:

- The rule conflicts with required behavior, public API, framework conventions,
