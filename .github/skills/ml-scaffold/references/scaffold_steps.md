...
`evaluate-ml-pipeline § Smoke`'s Stop condition forbids a test file before
the matching design note is approved, and no design note exists
at scaffold time.

Per-experiment placeholders land later via `evaluate-ml-pipeline § Smoke`
...
- `evaluate-ml-pipeline` § Smoke for the body of `tests/smoke/test_*.py`.
