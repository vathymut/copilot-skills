# Forbidden Shortcuts — evaluate-ml-pipeline § Audit
...
| Write `pixi add --feature agent ipython pyright` directly from this skill | Install commands owned by `python-stack-env`. This skill **requests** via G-AGENT-FEATURE; it does not install |
...
| `iterate-ml-experiment` § 4 dispatched audit → write scratch probe first to "double-check metrics" | The audit IS the metric-extraction step in § 4. Scratch probes for metrics are the anti-pattern this dispatch replaces |
