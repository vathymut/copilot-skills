...
**Why this beats manual/inline execution:** fresh context per task (no pollution), automatic two-stage review checkpoints, continuous progress. The cost is more subagent invocations and prep work — but issues surface early, cheaply.
...
When tasks are **fully independent** — different subsystems, different files, no shared state — dispatch one subagent per problem domain concurrently instead of task-by-task. Trigger: several unrelated test files or subsystems broken independently.

**Use when:** each problem can be understood without context from the others, and agents won't edit the same code. **Don't use when:** failures are related, you need full system state, or agents would touch shared files.

For the prompt structure, prompt smells, common mistakes, and a real worked example, see `references/parallel-and-inline.md` (Parallel Dispatch section).

## Inline Fallback (no subagents)

If the platform has no subagents, execute the plan yourself in-session. The full procedure (load/review plan, execute tasks in order, stop-and-ask on blockers, finish the branch) is in `references/parallel-and-inline.md` (Inline Fallback section). Never start implementation on main/master without explicit user consent.
